  ...
  if (req.method === 'POST') {
    try {
      ...
      for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        ...
        try {
          // 使用选中的账号调用 API
          const audioInfo = await (await sunoApi(account.cookie)).custom_generate(
            prompt, tags, title,
          ...
          if (circuitBreaker.shouldBreak(error)) {
            ...
            dbManager.addLog({
              ...
              operation: 'custom_generate_retry',
              status: 'failed',
      ...
      console.error('Unexpected error in custom_generate API:', error);
